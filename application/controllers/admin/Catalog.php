...
Class Catalog extends CI_controller {
	...
	public function do_add(){
		...
		if($img != 'ERROR'){
			...
			if($result){
				/*Update list catalog child*/
				$array_child = $this->Catalog_model->arrayChild($data['par_id']);
				$array_child =  json_encode($array_child);
				$this->Catalog_model->update_array_child($data['par_id'] ,$array_child);
	...
	public function do_edit(){
		...
		if($img != 'ERROR'){
			...
			$parentOld = (int)$this->input->post('parentOld');
			$code = generateSeoURL($this->input->post('txt_name'));
			$data 	= 	array(
				'name'			=>	addslashes($this->input->post('txt_name')),
				'par_id'		=>	addslashes($this->input->post('cbo_par')),
			...
			if($result){
				/*Update old parent*/
				$array_child = $this->Catalog_model->arrayChild($parentOld);
				$array_child =  json_encode($array_child);
				$this->Catalog_model->update_array_child($parentOld ,$array_child);
